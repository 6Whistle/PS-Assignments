...
class Huffman{
    ...
    map<char, map<char, pair<uint8_t, bitset<16> > > > charTable;     //Character's Huffman table
    ...
    void Make(map<char, pair<int, map<char, int> > > prob){
        Tree bst;
    ...
    void printTable(){
        for(auto i : table){
            cout << "Table >> " << i.first << " : size-" << (unsigned int)(i.second.first) << ", pattern-" << i.second.second << endl;
    ...
    void encoding(const char* input_file_path){
        ...
        for(auto i : table){        //Write all table element
            ...
            /* input size of Character Huffman */
            temp = (unsigned int)charTable[i.first].size() << 24;
            t_size = 8;
            if(max + t_size > 32){      //if buffer + temp size > 32 bits
                unsigned int b = temp >> max;       //write 32 bit buffer
                if(max == 32)
            ...
            /* input Character Huffman */
            for(auto j : charTable){
                temp = (unsigned int)(i.first) << 8;       //temp = [character|size|pattern]
                temp = (temp | (unsigned int)(i.second.first)) << 16;
                temp = temp | (unsigned int)(i.second.second.to_ulong());
                t_size = 16 + (int)(i.second.first);                //temp's size
...
int main(){
    ...
    char c, c_before;
    if(input_file.get(c).eof()){
        printf("No Data in traing file");
        return -1;
    }
    (next_ascii_count[c].first)++;
    c_before = c;
    while(!input_file.get(c).eof()){               //count each character's probability in input_data
        next_ascii_count[c_before].second[c]++;
        (next_ascii_count[c].first)++;
        c_before = c;
    }
    next_ascii_count[c_before].second[127]++;
    input_file.close();
